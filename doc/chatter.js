(function() {
  ...
  Chatter.Like = (function() {
    ...
    Like.renderLikesText = function(likes, likesTotal, isLikedByCurrentUser, instance_url) {
      var arr, decrementer, el, html, i, like, num, totalToAdd, userLink, _i, _j, _len, _len1;
      if (likes.length === 0) {
        return "";
      }
      arr = [];
      ...
      for (i = _i = 0, _len = likes.length; _i < _len; i = ++_i) {
        like = likes[i];
        if (i < totalToAdd) {
          if (like.user.id !== $.cookie('user_id')) {
            arr.push(userLink(like.user));
          }
        }
      }
      decrementer = 1;
    ...
    Like.unLikeAction = function(target, objectName) {
      ...
      return $.ajax({
        ...
        success: function() {
          ...
          for (i = _i = 0, _ref = item.likes.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
            if (item.likes[i].id === likeId) {
